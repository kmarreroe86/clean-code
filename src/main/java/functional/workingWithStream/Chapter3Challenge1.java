...
public class Chapter3Challenge1 {
    ...
    public static void main(String[] args) {
        ...
        // Answer 1 goes here. Return list containing only people names
        Function<Person, String> personNameMap = (person) -> person.name;
        List<String> peopleNameList = people.stream()
//                .map(p -> p.name)
                .map(personNameMap)
                .collect(Collectors.toList());
        System.out.println("People names: " + peopleNameList);
        ...
        // Answer 2 goes here. Return only the blue cars
        Predicate<Car> blueCarPredicate = (car) -> "blue".equals(car.color);
        List<Car> blueCars = cars.stream()
//                .filter(car -> "blue".equals(car.color))
                .filter(blueCarPredicate)
                .collect(Collectors.toList());
        ...
        // Answer 3 goes here. Return the sum of all salaries
        BinaryOperator<Float> totalSalaryOfEmployees = (acc, emp) -> {
          Float result = acc + emp;
          return result;
        };

        Float totalSalary = employees.stream()
                .map(emp -> emp.salary)
//                .reduce(0f, (salAcc, sal) -> salAcc + sal);
                .reduce(0f, totalSalaryOfEmployees);
